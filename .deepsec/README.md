...
Currently configured project: `sycom-learn` (target: `..`).
...
original init command. Checkpoints in `data/sycom-learn/setup/setup-state.json`
...
`--project-id sycom-learn` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/sycom-learn/`.
